...
PAGE_DFA("/service/upnpav", EWF_ACCESS_PRIVATE);
...
DISPLAY_HANDLER
{
        ...
        if( nbd_usharectl_status(&buf_xml, &buf_xml_size) == NBD_SUCCESS )
        {
                ...
                if(xmln_root != NULL)
                {
                        ewf_hdf_set("upnpav_active", ezxml_child_txt(xmln_root, "active"));
                        ewf_hdf_set("upnpav_status", ezxml_child_txt(xmln_root, "status"));
                        ewf_hdf_set("upnpav_errno", ezxml_child_txt(xmln_root, "errno"));
                        ewf_hdf_set("upnpav_sharedcount", ezxml_child_txt(xmln_root, "sharedcount"));
        ...
        nv_to_hdf("upnpav_name", "upnpav_name");

        /* get list of shares */
        if ( nbd_user_config_xml("upnpav", &buf_xml, &buf_xml_size) == 0 )
        {
                xmln_root = ezxml_parse_str(buf_xml, buf_xml_size);
                      ...
                      n++, xmln = xmln->next )
                {
                        ...
                        nbu_string_printf(buffer, sizeof buffer,
                                          "upnpav_stockage_list.%d.uuid",
                                          n);
                        ewf_hdf_set(buffer, uuid);

                        /* dir */
                        nbu_string_printf(buffer, sizeof buffer,
                                          "upnpav_stockage_list.%d.dir",
                                          n);
                        ewf_hdf_set(buffer,
...
FORM_HANDLER {
        ...
        if(action != NULL)
        {
                ...
                {
                        stock_share = ewf_hdf_sget("upnpav_stockage_share");
                        ...
                        if(stock_share != NULL)
                        {
                                   ...
                                   && nbu_list_count(list_arg) == 2)
                                {
                                        ...
                                        if(nbd_usharectl_add_share(stock_uuid, stock_dir) != NBD_SUCCESS)
                                        {
                                                nbu_log_error("Error !");
                                ...
                                {
                                        nbu_log_error("Invalid upnpav_stockage_share ! it's not normal guy !");
                                        ewf_hdf_form_set_error("unknown");
                                }
                ...
                {
                        idx = ewf_hdf_sget("upnpav_share_idx");
                        ...
                        if(idx != NULL)
                        {
                                ...
                                if(errno != 0 || nbd_usharectl_del_share(i) != NBD_SUCCESS)
                                {
                                        nbu_log_error("Error !");
                                        ewf_hdf_form_set_error("unknown");
                                }
                ...
                {
                        if(hdf_to_nv("upnpav_active", "upnpav_active") == NV_SET_SUCCESS)
                        {
                                restart++;
                        }

                        if(hdf_to_nv("upnpav_name", "upnpav_name") == NV_SET_SUCCESS)
                        {
                                restart++;
                        }
                }
...
AJAX_HANDLER
{
        ...
        if( nbd_usharectl_status(&buf_xml, &buf_xml_size) == NBD_SUCCESS )
        {
                ...
                if(xmln_root != NULL)
                {
                        ewf_hdf_set("upnpav_active",
                                    ezxml_child_txt(xmln_root, "active"));

                        ewf_hdf_set("upnpav_status",
                                    ezxml_child_txt(xmln_root, "status"));

                        ewf_hdf_set("upnpav_errno",
                                    ezxml_child_txt(xmln_root, "errno"));

                        ewf_hdf_set("upnpav_sharedcount",
                                    ezxml_child_txt(xmln_root, "sharedcount"));
